...
def calc_iou(R, img_data, class_mapping):
    ...
    (resized_width, resized_height) = data_generators.get_new_img_size(width, height, 600)
    ...
    for ix in range(R.shape[0]):
        ...
        for bbox_num in range(len(bboxes)):
            curr_iou = intersection_over_union.iou([gta[bbox_num, 0], gta[bbox_num, 2], gta[bbox_num, 1], gta[bbox_num, 3]],
                                           [x1, y1, x2, y2])
...
def non_max_suppression_fast1(boxes, probs, overlap_thresh=0.9, max_boxes=300):
